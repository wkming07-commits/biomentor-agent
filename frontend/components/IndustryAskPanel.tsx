...
import { Send, Sparkles, RefreshCw, BookOpen, Lightbulb, Target, Compass, Search } from "lucide-react";
    ...
    <div className="w-full">
      <form onSubmit={handleSubmit} className="relative mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-12 pl-11 pr-28 rounded-2xl glass-card text-sm font-body text-brand-ink placeholder:text-brand-faint/50 outline-none focus:border-accent-electric/30 transition-all duration-200"
          />
          <button
            type="submit"
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {quickTags.map((tag) => (
          <button
            ...
            className="px-3 py-1.5 rounded-full text-xs font-medium font-body border border-black/5 bg-white/40 text-brand-muted hover:text-brand-ink hover:border-accent-electric/20 hover:bg-white/70 transition-all cursor-pointer"
          >
            {tag}
          </button>
        ))}
      ...
      {loading && (
        <div className="glass-card rounded-2xl p-10 text-center">
          <Sparkles className="w-8 h-8 text-accent-electric mx-auto mb-3 animate-shimmer" />
      ...
      {answer && !loading && (
        <div className="glass-card rounded-2xl p-6 space-y-5 animate-reveal-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <ul className="space-y-0.5">
                {answer.researchFrontiers.map((rf, i) => (
                  <li key={i} className="text-xs text-brand-muted font-body pl-3 relative before:content-[''] before:absolute before:left-0 before:top-[7px] before:w-1 before:h-1 before:rounded-full before:bg-accent-electric/40">
                    {rf}
                  </li>
                ))}
              </ul>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-1.5">
                <Target className="w-3.5 h-3.5 text-accent-rose" />
                <h4 className="text-xs font-bold text-brand-ink uppercase tracking-wider">产业应用场景</h4>
              </div>
              <ul className="space-y-0.5">
                {answer.industryApplications.map((ia, i) => (
                  <li key={i} className="text-xs text-brand-muted font-body pl-3 relative before:content-[''] before:absolute before:left-0 before:top-[7px] before:w-1 before:h-1 before:rounded-full before:bg-accent-cyan/40">
                    {ia}
                  </li>
                ))}
              </ul>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-1.5">
                <Compass className="w-3.5 h-3.5 text-accent-electric" />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {answer.recommendedKeywords.map((kw, i) => (
                  <code key={i} className="text-[10px] font-mono text-accent-electric bg-accent-electric/5 px-1.5 py-0.5 rounded">
                    {kw}
                  </code>
                ))}
              </div>
              ...
              <ul className="space-y-1">
                {answer.researchTasks.map((rt, i) => (
                  <li key={i} className="text-xs text-brand-muted font-body flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded bg-accent-electric/10 text-accent-electric text-[9px] font-bold flex items-center justify-center shrink-0">
                      {i + 1}
                    </span>
                    {rt}
                  </li>
